...
{ 
  leds->led1 = HAL_GPIO_ReadPin(LED_BUS1_GPIO_Port,LED_BUS1_Pin);
	leds->led2 = HAL_GPIO_ReadPin(LED_BUS2_GPIO_Port,LED_BUS2_Pin);
}

void my_set_leds(struct leds *leds)
{
  HAL_GPIO_WritePin(LED_BUS1_GPIO_Port,LED_BUS1_Pin,leds->led1);
  HAL_GPIO_WritePin(LED_BUS2_GPIO_Port,LED_BUS2_Pin,leds->led2); 
}
...
//  #endif
  uint8_t ip_add[4]={192, 168, 10, 10};
...
{		   
  Log("Mongoose Task Start\r\n");
    ...
    /* Create the task. */
    if( xTaskCreate(
    vTask_Mongoose2, /* Pointer to the function that implements the task. */
