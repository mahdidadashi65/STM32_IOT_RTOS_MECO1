...
#elif MG_HAL_SYSTICK != MG_HAL_SYSTICK_NONE && MG_HAL_SYSTICK != MG_HAL_SYSTICK_CUBE
...
static void hal_rng_init(void);
static uint32_t hal_rng_read(void);
#endif
...
// mock ST hardware-releated function in system_.c, called from startup_*.s
// we handle this stuff in clock_init()
...
void hal_init(void) {
  ...
  hal_clock_init();                            // Set system clock to SYS_FREQUENCY
  SystemCoreClock = SYS_FREQUENCY;         // Update SystemCoreClock global var
#endif

#if MG_HAL_SYSTICK == MG_HAL_SYSTICK_CUBE
  HAL_InitTick(15);                       // Init Systick for Cube, priority 15
#elif MG_HAL_SYSTICK != MG_HAL_SYSTICK_NONE
  SysTick_Config(SystemCoreClock / 1000);  // Sys tick every 1ms
#endif

#ifndef MG_HAL_DISABLE_RANDOM
  hal_rng_init();                              // Initialise random number generator
#endif

#ifndef MG_HAL_DISABLE_UART
  hal_uart_init(UART_DEBUG, 115200);  // Initialise UART
#ifdef MG_HAL_ARCH_ARMCGT
  _stream[1].dfd = 1;
  ...
  hal_ethernet_init();                // Initialise Ethernet pins
#endif
}

#ifdef MG_HAL_ENABLE_GENMAC
void hal_genmac(unsigned char *mac) {
  hal_ethernet_genmac(mac);
}
#endif
...
static void hal_rng_init(void) {
  s_hal_rng_seed = hal_entropy_init();
}

static uint32_t hal_rng_read() {
  uint32_t seed = s_hal_rng_seed;
  uint32_t ent;
  if (hal_entropy_get(&ent)) {
...
int fputc(int c, FILE *stream) {
  if (stream == stdout || stream == stderr) hal_uart_write_byte(UART_DEBUG, c);
  return c;
}
